...
<%--
	* jsp:include
		- include 지시어와 유사하지만 동작 시점이 다름
			(include 지시어 는 컴파일 시점에 코드가 합쳐지는 방식, 정적)
		- 액션태그 include 는 요청이 들어 올 때마다 해당 페이지를 실행 한 결과를 삽입 (동적)
					=> 페이지의 로직이 매 요청마다 실행되므로 변동 데이터 표시에 적합
	
	* jsP:forward
		- 서버 내부에서 다른 jsp 페이지로 제어를 넘기는 방식
		- 클라이언트 모르게 서버에서만 처리 페이지가 바뀜
				=> 브라우저 URL은 바뀌지 않음
				--> request 객체가 그대로 유지되므로 파라미터는 전달 됨
--%>
<!DOCTYPE html>
<html>
<head>
