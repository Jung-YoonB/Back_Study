 ...
 * 	[1] init()		: 최초 요청 시 딱 1번만 호출 됨 (객체 생성 시점)
 * 			- 초기화 작업 (DB 연결, 설정 로드 등)을 이 메소드에 작성
 * 			- 이후 요청부터는 이미 만들어진 객체를 재사용 (싱글톤)
 * 
 * 	[2] service()	: 요청이 들어 올 때마다 매번 호출 됨
 * 			- HTPP 요청 방식 (GET/POST 등)을 판단하여 doGet(). doPost() 등으로 자동 분기함
 * 			- 우리가 직접 오버라이딩 하지 않아도 HPPTServlet이 처리해줌 * 
 * 
 * 	[3] destroy()	: 서버가 종료되거나 서블릿이 컨테이너에서 제거될 때 딱 1번만 호출 됨
...
public class LifeCycleServlet extends HttpServlet {
	...
	/**
	 * 서블릿 소멸 메소드
	 * 서버 종료 또는 서블릿 언로드(해제) 시 딱 1번만 호출 됨
	 */
	@Override
	public void destroy() {
