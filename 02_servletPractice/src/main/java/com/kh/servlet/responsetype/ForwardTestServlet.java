...
/**
 * * forward : 서버 내부 전달
 * 		- 서블릿이 요청을 받아 처리 한 뒤, 화면 응답을 다은 자원(jsp 등)에게 넘기는 방식
 ...
 * 		- 단, 같은 요청 안에서만 데이터가 유지 됨 (요청이 새로 들어오면 데이터는 사라짐)
 * 
 * * 처리 위치		: 서버 내부
 * * URL 변경		: 변경 되지 않음 (최초 요청 그대로 유지)
 * * 요청 횟수		: 1번
...
public class ForwardTestServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.setAttribute("message", "서블릿에서 저장 한 데이터 @@");
