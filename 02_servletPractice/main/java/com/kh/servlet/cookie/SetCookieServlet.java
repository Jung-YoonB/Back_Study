 ...
 * 		- 브라우저는 쿠키를 받으면 로컬에 저장 해두고, 이후 동일한 서버로 요청을 보낼 때마다 자동으로 함께 전송
 * 	=> 데이터가 클라이언트쪽에 저장 됨
 * 
 * * 쿠키 활용 예) 자동 로그인, 최근 본 상품, 팝업 "오늘 하루 보지 않기" 등
 * 		-> 클라이언트에 저장되므로 사용자가 임의로 수정하거나 탈취될 수 잇음
...
public class SetCookieServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Cookie cookie = new Cookie("nickname", "Ssac");
		
		// - 유효 시간 설정(단위 : 초)
		//		setMaxAge(시간)
		//			= 1시간 설정 : 3600 / 60*60
		cookie.setMaxAge(3600);
			// => 설정하지 않는 경우 브라우저를 닫는 순간 쿠키는 삭제 됨(세션 쿠키)
